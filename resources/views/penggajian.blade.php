    ...
    <div class="container mt-3">
        ...
        <form action="{{ route('gaji.hitung') }}" method="POST">
            ...
            <div class="mb-3 col-md-6">
                ...
                <select id="pegawai" name="pegawai_id" class="form-select" required>
                    ...
                    @foreach ($pegawaiList as $pegawaiItem)
                        <option value="{{ $pegawaiItem->id }}" data-gaji="{{ $pegawaiItem->gaji }}"
            ...
            <div class="mb-3 col-md-6">
                ...
                <input type="number" name="potongan_pajak" id="potongan_pajak"
                    class="form-control @error('potongan_pajak') is-invalid @enderror"
                    value="{{ old('potongan_pajak', $potonganPajakPersen ?? 0) }}" step="0.1" min="0"
                    max="100">
            ...
            <div class="mb-3 col-md-6">
                ...
                <select id="periode" name="periode" class="form-select @error('periode') is-invalid @enderror" required>
                    ...
                    <option value="2" {{ old('periode') == '2' ? 'selected' : '' }}>Periode 2 (Tanggal 16 - Akhir
                        Bulan)</option>
                </select>
                @error('periode')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="mb-3 col-md-6">
                ...
                <input type="text" name="insentif" id="insentif"
                    class="form-control @error('insentif') is-invalid @enderror" value="{{ old('insentif', 0) }}"
                    min="0" required>
                @error('insentif')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr>
                        <td>{{ $jumlahIzin ?? 0 }}</td>
                        <td>{{ $jumlahTidakHadir ?? 0 }}</td>
                        <td>{{ isset($potonganAbsen) ? number_format($potonganAbsen, 0, ',', '.') : 0 }}</td>
                        <td>{{ isset($potonganBPJS) ? number_format($potonganBPJS, 0, ',', '.') : 0 }}</td>
                        <td>{{ $potonganPajakPersen ?? 0 }}%</td>
                        <td>{{ isset($totalPotongan) ? number_format($totalPotongan, 0, ',', '.') : 0 }}</td>
                    </tr>
                </tbody>
            </table>
        ...
        @isset($totalGaji)
            ...
            <div id="slip-gaji-content">
                <div style="text-align: center; margin-bottom: 10px;">
                    <h1
                        style="font-family: 'Times New Roman', Times, serif; color: orange; margin: 0; font-weight: bold; font-style: italic;">
                        PT. Sarina Indika Makmur
                    </h1>
                    <br>
                    <p style="margin: 0; font-family: 'Times New Roman', Times, serif;">JL. Jend Sudirman No.41/49 A Palembang
                    </p>
                    <p style="margin: 0; font-family: 'Times New Roman', Times, serif;">Telp (0711) 313414/310562 Fax (0711)
                        312226</p>
                ...
                <table class="table table-bordered mt-4" style="width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>BPJS</td>
                            <td>Rp. {{ number_format($potonganBPJS, 0, ',', '.') }}</td>
                        </tr>
                        <tr>
                            <td>Potongan Pajak</td>
                            <td>{{ $potonganPajakPersen }} % (Rp. {{ number_format($potonganPajak, 0, ',', '.') }})</td>
                        </tr>
                        <tr>
                            <td class="text-end"><strong>Total Pengurangan</strong></td>
                            <td><strong>Rp {{ number_format($totalPotongan, 0, ',', '.') }}</strong></td>
                        </tr>
                        <tr>
        ...
        <script>
            function printSlip() {
                ...
                printWindow.document.write(
                    'table, th, td { border: 1px solid black; padding: 8px; text-align: left; }</style>');
